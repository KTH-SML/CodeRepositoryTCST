...
def calcPoseError(current_pose, goal_pose):
    position_error = sqrt((current_pose.position.x - goal_pose.position.x)**2 + (current_pose.position.y - goal_pose.position.y)**2 + (current_pose.position.z - goal_pose.position.z)**2)
    current_euler = euler_from_quaternion([current_pose.orientation.x, current_pose.orientation.y, current_pose.orientation.z, current_pose.orientation.w])
    goal_euler = euler_from_quaternion([goal_pose.orientation.x, goal_pose.orientation.y, goal_pose.orientation.z, goal_pose.orientation.w])
...
def PoseCallback(posedata):
    ...
    global robot_pose_3D

    robot_pose_3D = posedata
        ...
        if active == True:
            try:
                ...
                if isinstance(next_move, str):
                    ...
                    print(robot_pose_3D)
                    goal_pose_error = calcPoseError(robot_pose_3D.pose.pose, navi_goal.target_pose.pose)
                    while (not rospy.is_shutdown()) and (navigation.get_state() != GoalStatus.SUCCEEDED) and (goal_pose_error[0] > 0.15):
                        print(goal_pose_error)
                        try:
